...
describe("useEntityData", () => {
  ...
  it("no-ops updateEntity when there is no entity yet", async () => {
    const { result, axiosPrivate } = renderHookWithAxios(() =>
      // No identifier provided: exercises the runtime no-entity path. The diverged
      // UseEntityDataProps union (#4, +`include`) has a required discriminant per branch, so the
      // empty object is cast to the props type — it stays {} at runtime.
      useEntityData({} as Parameters<typeof useEntityData>[0]),
    );
