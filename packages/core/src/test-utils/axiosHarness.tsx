...
/** A real store with just the `replyke` slice mounted — no RTK-Query `baseApi`. */
export function makeReplykeStore() {
  return configureStore({ reducer: { replyke: replykeReducers } });
}

export type ReplykeStore = ReturnType<typeof makeReplykeStore>;
...
/**
 * Renders a core hook wrapped in `ReplykeContext` plus a real (replyke-slice-only)
    ...
    return (
      <Provider store={store}>
        <ReplykeContext.Provider
          value={{ projectId, project } as ReplykeContextValues}
        >
          {children}
        </ReplykeContext.Provider>
      </Provider>
