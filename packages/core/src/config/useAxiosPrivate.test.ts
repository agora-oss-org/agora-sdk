...
/** Any request not yet retried (no `.sent` flag) gets a 401; retries succeed. */
// Agora divergence (#3): the reactive refresh triggers on HTTP 401 (token expired), not 403 —
// the Agora server reserves 403 for authorization denials. The stub mirrors that contract.
function stubRefreshableAdapter() {
  return vi.fn(async (config: InternalAxiosRequestConfig & { sent?: boolean }) => {
...
describe("useAxiosPrivate", () => {
  ...
  it("refreshes exactly once and retries with the new token when multiple requests 401 concurrently", async () => {
