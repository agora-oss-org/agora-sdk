...
const useReplykeDispatch = vi.fn();
const useReplykeSelector = vi.fn();
...
vi.mock("@agora-sdk/core", () => ({
  useProject: () => useProject(),
  useReplykeDispatch: () => useReplykeDispatch(),
  useReplykeSelector: (selector: unknown) => useReplykeSelector(selector),
...
describe("useOAuthSignIn (expo)", () => {
  beforeEach(() => {
    ...
    useReplykeDispatch.mockReturnValue(dispatch);
    useReplykeSelector.mockReturnValue(null);
  ...
  describe("linkOAuthProvider", () => {
    ...
    it("attaches the current access token when linking a provider", async () => {
      useReplykeSelector.mockReturnValue("access-token-1");
